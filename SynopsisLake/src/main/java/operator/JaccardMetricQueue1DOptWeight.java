...
public class JaccardMetricQueue1DOptWeight {
...
//	private int nextBoundaryId = 0;
	...
	public JaccardMetricQueue1DOptWeight(double quality, int targetSize, int totalSrcIntervalSize, double min, double max) {
...
//		this.totalSrcIntervalSize = totalSrcIntervalSize;
	...
	public MyPriorityQueue<JMBoundary1D> reshapping(ArrayList<double[]> sources, ArrayList<double[]> weights) {
//		ArrayList<JMBoundary1D> opt = optimalSolution(sources, weights);
		
		long startTime = System.nanoTime();
		ArrayList<JMBoundary1D> opt = optimalSolution(sources, weights);
		long endTime = System.nanoTime();
		double generateOpt = (endTime - startTime) * 1E-9;
//		System.out.println("generate opt solution time = " + generateOpt + " s.");
		time += generateOpt;

		return reduceNumInterval(opt, sources);
...
//		System.out.println("num of initial opt boundary = " + opt.size());
		...
		long endTime = System.nanoTime();
		double initQueue = (endTime - startTime) * 1E-9;
		time += initQueue;
//		System.out.println("initial target queue time = " + initQueue + " s.");
...
//			System.out.println("pop boundary = " + boundary + ", left = " + leftB + ", right = " + rightB);
		...
		double dropTime = (endTime - startTime) * 1E-9;
		time += dropTime;
	
//		System.out.println("drop boundary time = " + dropTime + " s.");
...
//		opt.get(lastIdx).computeScore(idToInterval, overlapSrInfos.get(lastIdx), new HashSet<Integer>(), -1, 0);
	...
	private ArrayList<JMBoundary1D> optimalSolution(ArrayList<double[]> sources, ArrayList<double[]> weights) {
...
//		System.out.println("numSrc = " + numSrc);
		...
		for (int srcId = 0; srcId < numSrc; srcId++) {
			idxs[srcId] = 1;
			OptSrcEndBoundary srcBoundary = new OptSrcEndBoundary(srcId, sources.get(srcId)[0], weights.get(srcId)[0]);
			rightBoundaries.add(srcBoundary);
			numSrcBoundaryInOneSrc[srcId] = sources.get(srcId).length;
		}
...
//				System.out.println("insert " + optB);
			...
			if (nextId < numSrcBoundaryInOneSrc[optBoundaryCandidate.srcId]) {
				OptSrcEndBoundary srcBoundary = new OptSrcEndBoundary(optBoundaryCandidate.srcId,
						sources.get(optBoundaryCandidate.srcId)[nextId], weights.get(optBoundaryCandidate.srcId)[nextId]);
				rightBoundaries.add(srcBoundary);
				idxs[optBoundaryCandidate.srcId] = nextId + 1;
			}
			// create a src interval
			if (nextId > 1) {
				JMSrcInterval src = new JMSrcInterval(srcIId, weights.get(optBoundaryCandidate.srcId)[nextId-1],
