...
//		int totalFileSizeInBytes = 0;
		...
		try {
			...
			for (int i = 1; i < lines.size(); i++) {
				...
				if (haveSynopsis && (synopsisType == 0 || synopsisType == 1 || 
						synopsisType == 2 || synopsisType == 3)) {
				...
				if (haveSynopsis && (synopsisType == 4)) {
...
//				waveletsRow.setCoeffStringArray(coeffStringArrays);
			...
			case 6:
				...
				uniformSamples.loadHistByDataList(uniformSamples, dataList);
				uniformSamplesRow.setSampleRatio(uniformSampleRate);
				uniformSamplesRow.setSampleData(uniformSamples.getSampleData());
	...
	public void buildFileWithoutData(List<String> lines) {
